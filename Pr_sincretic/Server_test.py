...
def hello_world():
    ...
    temp_set = '<p>Seteaza temperatura dorita: <form method=\"get\" action=\"temp\"><input name=\"tmp\" type=\"number\" value=\"25\" min=\"25\" max=\"80\"/> <input type=\"submit\" value=\"send\"></form></p>'
    
    return text + temp + temp_serial + string_butoane + color_picker + text_form + temp_set
...
@app.route('/temp')
def temp_send():
    temp_setat = str(request.args['tmp'])
    temp_serial = "*" + temp_setat + ")"
    print(temp_serial)
    ser.write(temp_serial.encode())
    return "Am trimis temperatura"
